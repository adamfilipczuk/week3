...
function filterNegativeNumbers(numbers) {
  let number = numbers.filter(x => x > -1);
  return number;
}
...
function doubleDivisibleByThree(numbers) {
  let number = numbers.filter(x => x % 3 === 0);
...
function selectHighPerformingStudents(students) {
  ...
  let student = students.filter(student => student.GPA >= 5);
  student = student.filter(student => student.hobbies.includes("coding"));

  student = student.map(student => ({name: student.name, email: student.email}));

  student = student.sort((a,b) => a.name.localeCompare(b.name));
...
function aggregateStudentData(students) {
  ...
  //let studentAvgGPA = students.map(student => ({GPA: students.GPA}));
  let studentGPA = students.reduce((acc, students) => acc + students.GPA, 0);
  let studentAvgGPA = studentGPA / students.length;

  let codingStudentNum = students.filter(student => student.hobbies.includes("coding")).length;


  let codingStudent = students.filter(student => student.hobbies.includes("coding"));
  let codingStudentGPAT = codingStudent.reduce((acc, students) => acc + students.GPA, 0);
  let codingStudentGPA = codingStudentGPAT / codingStudentNum;

  return {
    studentNum,
    studentAvgGPA: Number(studentAvgGPA.toFixed(2)),
    codingStudentNum,
    codingStudentGPA: Number(codingStudentGPA.toFixed(2))
  };
